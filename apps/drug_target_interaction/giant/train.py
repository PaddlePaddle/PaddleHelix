...
"""
Training process code for Geometry-aware Interactive Graph Neural Networks (GIANT).
"""
import os
import time
import math
...
def evaluate(model, loader):
    ...
    for batch_data in loader:
        a2a_g, b2a_g, b2b_gl, mark_gl, feats, types, counts, y = batch_data
        _, y_hat = model(a2a_g, b2a_g, b2b_gl, mark_gl, types, counts)
...
def train(args, model, trn_loader, tst_loader, val_loader):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch_data in tqdm(trn_loader):
            a2a_g, b2a_g, b2b_gl, mark_gl, feats, types, counts, y = batch_data
            feats_hat, y_hat = model(a2a_g, b2a_g, b2b_gl, mark_gl, types, counts)

            # loss function
            loss = F.l1_loss(y_hat, y, reduction='sum')
            loss_inter = F.l1_loss(feats_hat, feats, reduction='sum')
            loss += args.lambda_type * loss_inter
            for layer in model.bond2bond_layers:
                w_g = paddle.stack([conv.G.weight for conv in layer.conv_layer])
                loss += args.lambda_domain * paddle.sum((w_g[1:, :, :] - w_g[:-1, :, :])**2)
            loss.backward()
            optim.step()
            optim.clear_grad()
            scheduler.step()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_dir', type=str, default='./output/gaint')
    parser.add_argument('--cuda', type=str, default='0')
    parser.add_argument('--seed', type=int, default=123)
    parser.add_argument("--save_model", action="store_true", default=True)

    parser.add_argument("--lambda_type", type=float, default=1.75)
    parser.add_argument("--lambda_domain", type=float, default=1e-4)
    ...
    parser.add_argument("--num_cross_layer", type=int, default=2)
    parser.add_argument("--num_inter_layer", type=int, default=1)
    ...
    args.mlp_dims = [eval(dim) for dim in args.mlp_dims.split(',')]
    if args.seed:
    ...
    model = GIANT(args)
    train(args, model, trn_loader, tst_loader, val_loader)7