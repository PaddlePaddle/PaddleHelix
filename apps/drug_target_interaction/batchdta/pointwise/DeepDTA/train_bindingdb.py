...
def training(model, training_loader, optim):
    ...
    """
    model.train()
    for _, (d_out, t_out, label, _) in enumerate(training_loader):
...
def predicting(model, testing_loader):
    ...
    with paddle.no_grad():
        ...
        for _, (d_out, t_out, test_label, test_group) in enumerate(testing_loader):
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        files = open("bestResult/DeepDTA_BindingDB_ki_result"+str(rounds)+".txt",'a')
