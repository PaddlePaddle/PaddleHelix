...
def training(model, training_loader, optim):
    ...
    """
    model.train()
    for _, (d_out, mask_d_out, t_out, mask_t_out, label) in enumerate(training_loader):
...
def predicting(model, testing_loader):
    ...
    with paddle.no_grad():
        ...
        for _,(d_out, mask_d_out, t_out, mask_t_out, label) in enumerate(testing_loader):
...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        files = open("bestResult/MolTrans_davis_result"+str(rounds)+".txt",'a')
        ...
        model_name = "bestModel/MolTrans_davis_"+str(rounds)+".model"
