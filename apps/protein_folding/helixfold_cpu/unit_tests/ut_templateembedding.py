...
model_name = 'tmp' #'templateembedding'
...
template_aatype = pd.ones([1, 4, len_dim], dtype='int32')
...
if not os.path.isfile(f_topo) or force_static_cvt:
  if is_dynamic_input:
    len_dim = None
  print('# [INFO] create static graph')
  net = to_static(model, input_spec=[
    InputSpec(shape=[1, len_dim, len_dim, 128], dtype='float32'),
    InputSpec(shape=[1, 4], dtype='float32'),
    InputSpec(shape=[1, 4, len_dim], dtype='int32'),
    InputSpec(shape=[1, 4, len_dim], dtype='float32'),
    InputSpec(shape=[1, 4, len_dim, 3], dtype='float32'),
    InputSpec(shape=[1, 4, len_dim, 37, 3], dtype='float32'),
    InputSpec(shape=[1, 4, len_dim, 37], dtype='float32'),
    InputSpec(shape=[1, 508, len_dim], dtype='float32'),
  ])
...
# 创建输入样例
print('# [INFO] create numpy samples')
query_embedding1 = np.ones([1, len_dim, len_dim, 128], dtype="float32")
template_mask1 = np.ones([1, 4], dtype="float32")
template_aatype1 = np.ones([1, len_dim], dtype="int32")
template_pseudo_beta_mask1 = np.ones([1, len_dim], dtype="float32")
template_pseudo_beta1 = np.ones([1, len_dim, 3], dtype="float32")
template_all_atom_positions1 = np.ones([1, len_dim, 37, 3], dtype="float32")
template_all_atom_masks1 = np.ones([1, len_dim, 37], dtype="float32")
mask_2d1 = np.ones([1, len_dim, len_dim], dtype="float32")

# 获取输入轴
print('# [INFO] get input handles')
