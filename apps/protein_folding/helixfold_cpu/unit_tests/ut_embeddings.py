import pdb
from layers.backbones import Embeddings
...
parser = Parser('[pd.infer] UT of pdpd.embeddings')
...
prefix_weights = 'dynamic_params/embeddings' if is_dynamic_input else 'static_params/embeddings'
...
batch = {
  ...
  'aatype': pd.ones([1, len_dim], dtype='int32'),
...
model = Embeddings(channel_num, c, gc)
model.eval()
if not os.path.isfile(f_topo):
  ...
  net = to_static(model, input_spec=[
    ...
    InputSpec(shape=[1, len_dim],name='aatype', dtype='int32'),
...
for i in tqdm(range(n_iter)):
  ...
  predictor.run()
  # msa_activations_raw_1 = outputls[output_names[0]].copy_to_cpu() # (1, 508, len, 256)
  # pair_activations = outputls[output_names[1]].copy_to_cpu() # (1, len, len, 128)
  # mask_2d = outputls[output_names[2]].copy_to_cpu() # (1, len, len)
