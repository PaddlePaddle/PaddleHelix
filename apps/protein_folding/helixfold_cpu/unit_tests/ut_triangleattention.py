import pdb
from layers.basics import TriangleAttention
...
c = cfg['model']['embeddings_and_evoformer']['evoformer']['triangle_attention_starting_node']
...
prefix_weights = 'dynamic_params/triangleattention' if is_dynamic_input else 'static_params/triangleattention'
...
model = TriangleAttention(channel_num, c, gc, 'triangle_attention')
model.eval()
...
# 获取输入轴
input_names = predictor.get_input_names() # ['pair_act', 'pair_mask]
inputl_a = predictor.get_input_handle('pair_act')
inputl_m = predictor.get_input_handle('pair_mask')

# 变形输入轴
if is_dynamic_input:
  print('# [INFO] re-organize dynamic axes')
  inputl_a.reshape(pair_act1.shape)
  inputl_m.reshape(pair_mask1.shape)
...
for i in tqdm(range(n_iter)):
  ...
  inputl_m.copy_from_cpu(pair_mask1)
  predictor.run()
