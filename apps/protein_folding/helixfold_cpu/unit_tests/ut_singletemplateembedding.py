from layers.basics import SingleTemplateEmbedding
...
n_warm = 3
n_iter = 10 + n_warm
force_static_cvt = False
ignore_eval = False
is_dynamic_input = False
model_name = 'singletemplateembedding'
...
model = SingleTemplateEmbedding(channel_num, c, gc)
model.eval()
if not os.path.isfile(f_topo) or force_static_cvt:
  if is_dynamic_input:
    len_dim = None
  net = to_static(model, input_spec=[
    InputSpec(shape=[1, len_dim], dtype='int32'),
    InputSpec(shape=[1, len_dim]),
    InputSpec(shape=[1, len_dim, 3]),
    InputSpec(shape=[1, len_dim, 37, 3]),
    InputSpec(shape=[1, len_dim, 37]),
...
template_aatype1 = np.ones([1, len_dim], dtype="int32") # 一定注意
