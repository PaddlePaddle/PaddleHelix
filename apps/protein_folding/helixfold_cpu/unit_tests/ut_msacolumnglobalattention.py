import pdb
from layers.basics import MSAColumnGlobalAttention
...
prefix_weights = 'dynamic_params/msacolumnglobalattention' if is_dynamic_input else 'static_params/msacolumnglobalattention'
...
### create sample input
len_dim = 764
msa_act = pd.ones([1, len_dim, 5120, 64])
msa_mask = pd.ones([1, len_dim,5120])

print('# [INFO] build and save static graph of Attention')
model = MSAColumnGlobalAttention(channel_num, c, gc)
model.eval()
if not os.path.isfile(f_topo):
  ...
  net = to_static(model, input_spec=[
    ...
    InputSpec(shape=[1, len_dim, 5120]),
  ])
...
msa_mask1 = np.ones([1, len_dim,5120], dtype='float32')
