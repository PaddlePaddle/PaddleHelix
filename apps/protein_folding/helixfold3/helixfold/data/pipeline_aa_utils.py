...
"""Utility functions for building the input features (reference ccd features) for the HelixFold model."""
 
import collections
from functools import partial
from typing import MutableMapping, Optional, Tuple, Union, List, Callable, Any
...
from helixfold.data.feature_processing import MAX_TEMPLATES as MAX_TEMPLATE_NUM
...
def make_intra_covalent_bonds(
        intra_coval_bonds: List[List[str]], 
        atom_ids: List[str], 
        num_tokens: int) -> Tuple[np.ndarray, np.ndarray]:
    """Process (intra)covalent bonds for non-standard CCD.
    
    Args:
        coval_bonds: List of bond information [left_atom_id, right_atom_id, bond_type]
        atom_ids: List of atom identifiers
        num_tokens: Number of tokens in the residue
        
    Returns:
        token_bonds: Binary matrix indicating bond connections
        bond_type: Matrix indicating bond types
    """
    token_bonds = np.zeros([num_tokens, num_tokens], dtype=np.float32)
    bond_type = np.zeros([num_tokens, num_tokens], dtype=np.int32)
    
    atom_id_to_token = {atom_id: idx for idx, atom_id in enumerate(atom_ids)}
    assert len(atom_id_to_token) == num_tokens, "Atom ID mapping mismatch"
    
    for bond_info in intra_coval_bonds:
        left_atom_id, right_atom_id, bond_type_str = bond_info

        left_token_idx = atom_id_to_token[left_atom_id]
        right_token_idx = atom_id_to_token[right_atom_id]
        token_bonds[left_token_idx, right_token_idx] = 1.0
        token_bonds[right_token_idx, left_token_idx] = 1.0
        
        bond_type_integer = ALLOWED_LIGAND_BONDS_TYPE.get(bond_type_str, 1)
        bond_type[left_token_idx, right_token_idx] = bond_type_integer
        bond_type[right_token_idx, left_token_idx] = bond_type_integer
    
    return token_bonds, bond_type
...
def merge_token_bonds_list(token_bonds_list: List[np.ndarray]) -> np.ndarray:
    """
...
def ccd_to_restype(ccd_id, chain_type):
    if ccd_id not in residue_constants.STANDARD_LIST:
        if chain_type in ['ligand', 'non_polymer', 'protein']:
            ...
            restype = residue_constants.HF3_restype_order['UNK']
        elif chain_type == 'dna':
            restype = residue_constants.HF3_restype_order['DN']
        elif chain_type == 'rna':
            restype = residue_constants.HF3_restype_order['N']
        else:
            raise ValueError(chain_type)
    else:
        restype = residue_constants.HF3_restype_order[ccd_id]
    return restype


def atom_perm_to_str_list(perm):
    """
    perm: [N_atom, N_perm]
    """
    return ["_".join([str(i) for i in p]) for p in perm]


def make_sequence_features(chain_id: str,
                        chain_type: str, 
                        ccd_seq: List[str], 
                        ccd_preprocessed_dict: dict) -> FeatureDict:
    """
    ...
    for residue_id, ccd_id in enumerate(ccd_seq):
        _ccd_feats = ccd_preprocessed_dict.get(ccd_id, None)
        assert _ccd_feats is not None, f'CCD <{ccd_id}> not in ccd_preprocessed_dict'
        ...
        features['ref_atom_name_chars'].append(np.array(
            [convert_atom_id_name(atom_id) 
                if is_standard else convert_atom_id_name("")
        ...
        features['chain_ids'].append(np.array(
                [chain_id] * num_tokens, dtype=object))  # [N_token]
        ...
        if 'atom_perm' in _ccd_feats:
            perm = _ccd_feats['atom_perm']
            if len(perm) != num_atoms:
                print(f'atom_perm error, num_atom miss match {ccd_id}. perm:{perm} num_atoms:{num_atoms}')
                perm = np.arange(num_atoms, dtype='int64').reshape(-1, 1) # (N_atom, 1)
        else:
            # print(f'missing atom_perm {ccd_id}')
            perm = np.arange(num_atoms, dtype='int64').reshape(-1, 1) # (N_atom, 1)
        features['atom_perm_str'].append(np.array(atom_perm_to_str_list(perm), dtype=object))

        ## token_bond features
        if not is_standard:
            # Process covalent bonds for non-standard ccd
            token_bonds, token_bonds_type = make_intra_covalent_bonds(
                _ccd_feats['coval_bonds'], 
                _ccd_feats['atom_ids'], 
                num_tokens
            )
        else:
            token_bonds = np.zeros([num_tokens, num_tokens], dtype=np.float32)
            token_bonds_type = np.zeros([num_tokens, num_tokens], dtype=np.int32)
        
        features['token_bonds'].append(token_bonds)
        features['token_bonds_type'].append(token_bonds_type)
    ...
    for k, v_list in features.items():
        if k in KEYS_TO_CONTINUOUS_INCREASE_BETWEEN_CHAINS:
            ...
            v = merge_value_list_to_continous_increase(v_list)
        elif k in ['token_bonds', 'token_bonds_type']:
...
def create_chain_features_without_real_msa(
        seq: str,
        chain_type: str,
        unk_map_fn: Callable[[str], str],
        parser_fn: Callable[[str], Any],
        sequence_featurizer: Callable[[str], Any],
        msa_featurizer: Callable[[Any], Any]) -> FeatureDict:
    """ create chain features without real msa. """
    seq = unk_map_fn(seq)
    temp_msa = parser_fn('', query=seq)
    msa_features = msa_featurizer((temp_msa,), max_align_depth=10)

    ## mock msa_all_seq features
    valid_feats = msa_pairing.MSA_FEATURES + (
        'msa_species_identifiers',
    )
    msa_features_for_pairing = {
        f'{k}_all_seq': v for k, v in msa_features.items() if k in valid_feats
    }

    num_res = len(seq)
    sequence_features = sequence_featurizer(
        sequence=seq,
        description='query',
        num_res=num_res)
    ...
    if chain_type == 'rna':
        raw_features = msa_pipeline_rna.final_feature_correction(raw_features)
...
def mock_msa_template_if_nonexist(
    ...
    if 'msa' not in raw_feats:
        raw_feats['msa'] = np.ones(
                ...
                ) * residue_constants.HF3_restype_order['-']
    ...
    if 'template_aatype' not in raw_feats:
        if before_msa_pairing:
            ...
            raw_feats["template_aatype"][:, :, residue_constants.HF3_restype_order['-']] = 1
        else:
...
def add_msa_extra_features(raw_feats: dict) -> FeatureDict:
...
def expand_msa_template_for_non_standard_residue(
        ...
        extra_feats: Optional[list]=None) -> FeatureDict:
    """
