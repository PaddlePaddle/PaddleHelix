...
TEMPLATE_FEATURES = ('template_aatype', 'template_all_atom_positions',
                     'template_all_atom_mask','template_all_atom_masks','template_domain_names')
#CHAIN_FEATURES = ('num_alignments', 'seq_length')
CHAIN_FEATURES = ('seq_length')


def create_paired_features(
    chains: Iterable[pipeline_a3m.FeatureDict]) ->  List[pipeline_a3m.FeatureDict]:
  ...
  if len(chains) < 2:
    ...
    #print('paired_chains_to_paired_row_indices:',paired_chains_to_paired_row_indices)
...
def _make_msa_df(chain_features: pipeline_a3m.FeatureDict) -> pd.DataFrame:
...
def pair_sequences(examples: List[pipeline_a3m.FeatureDict]
                   ) -> Dict[int, np.ndarray]:
...
def _correct_post_merged_feats(
    np_example: pipeline_a3m.FeatureDict,
    np_chains_list: Sequence[pipeline_a3m.FeatureDict],
    pair_msa_sequences: bool) -> pipeline_a3m.FeatureDict:
...
def _pad_templates(chains: Sequence[pipeline_a3m.FeatureDict],
                   max_templates: int) -> Sequence[pipeline_a3m.FeatureDict]:
...
def _merge_features_from_multiple_chains(
    chains: Sequence[pipeline_a3m.FeatureDict],
    pair_msa_sequences: bool) -> pipeline_a3m.FeatureDict:
...
def _merge_homomers_dense_msa(
    chains: Iterable[pipeline_a3m.FeatureDict]) -> Sequence[pipeline_a3m.FeatureDict]:
...
def _concatenate_paired_and_unpaired_features(
    example: pipeline_a3m.FeatureDict) -> pipeline_a3m.FeatureDict:
...
def merge_chain_features(np_chains_list: List[pipeline_a3m.FeatureDict],
...
def deduplicate_unpaired_sequences(
    np_chains: List[pipeline_a3m.FeatureDict]) -> List[pipeline_a3m.FeatureDict]:
