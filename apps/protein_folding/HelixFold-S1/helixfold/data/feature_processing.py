...
REQUIRED_FEATURES = frozenset({
    ...
    'template_all_atom_masks', 'template_all_atom_positions', 'template_all_atom_mask', 'template_domain_names'
})
...
def pair_and_merge(
    ...
    use_colabfold_search_feats: bool = False,
    ) -> pipeline.FeatureDict:
  """Runs processing on features to augment, pair and merge.

  Args:
    all_chain_features: A MutableMap of dictionaries of features for each chain.
    use_colabfold_search_feats: 
      whether use the colabfold_search feats which is already msa pairing.

  Returns:
    A dictionary of features.
  """

  if not use_colabfold_search_feats:
    ...
    process_unmerged_features_colabsearch(all_chain_features)
    np_chains_list = list(all_chain_features.values())
    pair_msa_sequences = not _is_homomer_or_monomer(np_chains_list)
    np_chains_list = crop_chains(
        np_chains_list,
    ...
    np_example = msa_pairing_a3m.merge_chain_features(
        np_chains_list=np_chains_list, pair_msa_sequences=pair_msa_sequences,
        max_templates=MAX_TEMPLATES)
    np_example = process_final_colabsearch(np_example)
...
def crop_chains(
    ...
    max_templates: int,
    use_colabfold_search_feats: bool = False) -> List[pipeline.FeatureDict]:
  ...
  for chain in chains_list:
    if not use_colabfold_search_feats:
      ...
      cropped_chain = _crop_single_chain_colabsearch(
        chain=chain,
        max_templates=max_templates)
...
def _crop_single_chain(chain: pipeline.FeatureDict,
  ...
  for k in chain:
    ...
    if k_split in msa_pairing.TEMPLATE_FEATURES:
      if k_split == 'template_domain_names':
        chain[k] = np.atleast_2d(chain[k]).T[:templates_crop_size, :]
      else:
...
def _crop_single_chain_colabsearch(chain: pipeline.FeatureDict,
                       max_templates: int) -> pipeline.FeatureDict:
  """Crops msa sequences to `msa_crop_size`."""
...
def process_final_colabsearch(np_example: pipeline.FeatureDict) -> pipeline.FeatureDict:
  """Final processing steps in data pipeline, after merging and pairing."""
  np_example = _make_seq_mask(np_example)
  np_example = _filter_features(np_example)
  return np_example
...
def process_unmerged_features_colabsearch(
    all_chain_features: MutableMapping[str, pipeline.FeatureDict]):
  """Postprocessing stage for per-chain features before merging."""
