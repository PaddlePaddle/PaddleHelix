...
class SeedMaker(object):
    ...
    def reset_init_seed(self, initial_seed):
        self.next_seed = initial_seed
...
def shaped_categorical(probs):
    """tbd."""
    ds = shape_list(probs) #20
    num_classes = ds[-1] # 20
    probs = np.reshape(probs, (-1, num_classes)) #1000*500, 20
    nums = list(range(num_classes)) #[1,20]
    counts = []
    for prob in probs:
        counts.append(np.random.choice(nums, p=prob)) #sampling
...
def gumbel_noise(shape):
    ...
    # uniform_noise = uniform(key, shape=shape, dtype=jnp.float32, minval=0., maxval=1.)
    uniform_noise = np.random.uniform(0., 1., shape)
    gumbel = -np.log(-np.log(uniform_noise + epsilon) + epsilon)
    return gumbel


def shaped_categorical_gumbel_max(probs):
    # key = np.random.default_rng(seed=None)
    z = gumbel_noise(probs.shape)
    ds = shape_list(probs)
    num_classes = ds[-1]
    probs = np.reshape(probs, (-1, num_classes))
    z = np.reshape(z, (-1, num_classes))
    return np.reshape(np.argmax(probs+z, axis=-1), ds[:-1])


def shaped_categorical_pd(probs, eps=1e-10):
      ds = probs.shape
      num_classes = ds[-1]
      probs = np.reshape(probs, (-1, num_classes))

      probs_t = paddle.to_tensor(np.log(probs + eps), place=paddle.CPUPlace())
      cat = paddle.distribution.Categorical(probs_t)
      counts = cat.sample([1])
      counts = counts.numpy()
      return np.reshape(np.array(counts, np.int32), ds[:-1])
...
def make_masked_msa(protein, config, replace_fraction):
    ...
    bert_msa = shaped_categorical_gumbel_max(categorical_probs)
    # bert_msa = shaped_categorical_pd(categorical_probs)
