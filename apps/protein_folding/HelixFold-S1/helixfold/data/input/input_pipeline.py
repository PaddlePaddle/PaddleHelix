...
"""Feature pre-processing input pipeline for HelixFold."""

import time
import copy
import numpy as np
import paddle
from helixfold.data.data_utils import compose
from helixfold.common import residue_constants
from helixfold.data.input import data_transforms
...
FEATURES = {
    ...
    'asym_id': (np.int64, [NUM_RES]),
    'sym_id': (np.int64, [NUM_RES]),
    'entity_id': (np.int64, [NUM_RES]),
    'entity_mask': (np.int64, [NUM_RES]),
}
...
def _make_features_metadata(feature_names, multimer_mode=False):
    ...
    multimer_features = ['asym_id', 'sym_id', 'entity_id', 'entity_mask']
    if multimer_mode:
        feature_names = list(set(feature_names + multimer_features))
...
def np_to_array_dict(np_example, features, use_templates=False, multimer_mode=False):
    """Creates dict of arrays."""

    np_example = copy.deepcopy(np_example)
    if multimer_mode:
        # TODO: make these changes in raw features.pkl
        np_example['aatype'] = data_transforms.one_hot(
            len(residue_constants.restypes_with_x), np_example['aatype'])

        np_example['template_aatype'] = data_transforms.one_hot(
        ...
        if len(np_example['seq_length'].shape) == 0:
            ...
            np_example['seq_length'] = np.ones([num_res, 1], dtype=np.int64) * \
                int(np_example['seq_length'])

        if len(np_example['num_alignments'].shape) == 0:
            num_res = np_example['aatype'].shape[0]
            np_example['num_alignments'] = np.ones([num_res, 1], dtype=np.int64) * \
                int(np_example['num_alignments'])

        if 'between_segment_residues' not in np_example:
            num_res = np_example['aatype'].shape[0]
            np_example['between_segment_residues'] = np.zeros((num_res,), dtype=np.int32)

    features_metadata = _make_features_metadata(
        features, multimer_mode=multimer_mode)
    array_dict = {k: v for k, v in np_example.items()
                  if k in features_metadata}

    if 'num_templates' in np_example:
        num_template = int(np_example['num_templates'])
    elif 'template_domain_names' in np_example:
