...
import functools
import io
from typing import Any, Mapping, Optional, Sequence, Tuple, List, Union
...
def _get_polymer_chains(
  ...
  ## TODO: convert dna/rna hybrid to one type, [Proportion dependent]. Now is delete this hybrid chainID.
...
def _get_covalent_bond_info(
  ...
  Args:
    parsed_info: _mmcif_dict produced by the Biopython parser.
    ## NOTE: AF3 is only consider Token-level distance (_struct_conn.pdbx_dist_value： distance between two atoms)
...
def _residue_basic_clean(ResAtPos: ResidueAtPosition) -> ResidueAtPosition:
  ## Residue level; basic structure clean up, AF3:
...
def delete_chain(input_file, output_file, chain_to_delete):
   
  parser = MMCIFParser(QUIET=True)
  structure = parser.get_structure("structure", input_file)
  for model in structure:
    chains_to_remove = [chain.id for chain in model if chain.id == chain_to_delete]
...
if __name__ == '__main__':
  import os
  import gzip
  f='data_preprocess/raw_data/debug/assembly/1hho-assembly1.cif'
  
  if f.endswith(".gz"): # cif.gz
      with gzip.open(os.path.join(f), "rb") as fp:
          mmcif_string = fp.read().decode('utf-8')
  else:
      with open(os.path.join(f), "r") as fp:
          mmcif_string = fp.read()
  file_id = os.path.splitext(f)[0]

  t = parse(file_id=file_id, mmcif_string=mmcif_string)
  import pdb
  pdb.set_trace()
  print(t.errors)