...
REQUIRED_FEATURES = frozenset({
    ...
    'template_all_atom_mask', 'template_all_atom_positions','template_all_atom_masks'
})
...
def _is_homomer_or_monomer(chains: Iterable[pipeline_a3m.FeatureDict]) -> bool:
...
def pair_and_merge(
  ...
  logging.info('after process unmerged feature')
  ...
  if pair_msa_sequences:
    np_chains_list = msa_pairing_a3m.create_paired_features(
        chains=np_chains_list)
    np_chains_list = msa_pairing_a3m.deduplicate_unpaired_sequences(np_chains_list)
  '''
  ...
  np_example = msa_pairing_a3m.merge_chain_features(
...
def crop_chains(
    chains_list: List[pipeline_a3m.FeatureDict],
    msa_crop_size: int,
    pair_msa_sequences: bool,
    max_templates: int) -> List[pipeline_a3m.FeatureDict]:
...
def _crop_single_chain(chain: pipeline_a3m.FeatureDict,
                       msa_crop_size: int,
                       pair_msa_sequences: bool,
                       max_templates: int) -> pipeline_a3m.FeatureDict:
  ...
  if pair_msa_sequences:
    ...
    num_non_gapped_pairs = np.sum(
        np.any(msa_all_seq != msa_pairing_a3m.MSA_GAP_IDX, axis=1))
  ...
  for k in chain:
    ...
    if k_split in msa_pairing_a3m.TEMPLATE_FEATURES:
      chain[k] = chain[k][:templates_crop_size, :]
    '''
    elif k_split in msa_pairing_a3m.MSA_FEATURES:
...
def process_final(np_example: pipeline_a3m.FeatureDict) -> pipeline_a3m.FeatureDict:
...
def _filter_features(np_example: pipeline_a3m.FeatureDict) -> pipeline_a3m.FeatureDict:
...
def process_unmerged_features(
  ...
  for chain_features in all_chain_features.values():
    ...
    '''
    # 1>>all_atom_mask 2>all_atom_mask 3>all_atom_posintion 4.assembly_num_chain
    ...
    '''
    #这部分因为训练需要不再像推断那样设为0，在pipeline里提取了真实值，这里就不再提取
