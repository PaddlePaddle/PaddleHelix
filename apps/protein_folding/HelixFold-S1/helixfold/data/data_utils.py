...
from helixfold.common import residue_constants, protein
...
_MSA_FEATURE_NAMES = [
    'msa', 'deletion_matrix', 'cluster_bias_mask', 'msa_mask'
]
...
def AFDB_a3m_to_msa_features(a3m_string):
    """MSA to af2 processed features."""

    msa_object = parsers.parse_a3m(a3m_string)
    input_sequence = msa_object.sequences[0]
    input_description = "xxx"   # TODO: description missing
    num_res = len(input_sequence)
    sequence_features = make_sequence_features(
            sequence=input_sequence,
    ...
    msa_features = make_msa_features([msa_object])
    raw_features = {**sequence_features, **msa_features}
...
def sample_raw_msa(raw_features, min_depth=None, max_depth=None):
    """
    related key in raw_features:
        msa: (msa_depth, seq_len)
        deletion_matrix_int: (msa_depth, seq_len)
        msa_species_identifiers: (msa_depth,)
        num_alignments: (seq_len,)
        msa_all_seq: (msa_depth_all_seq, seq_len)
        deletion_matrix_int_all_seq: (msa_depth_all_seq, seq_len)
        msa_species_identifiers_all_seq: (msa_depth_all_seq,)
    """
    def _get_random_indices(real_depth, min_depth, max_depth):
        new_depth = np.random.randint(min_depth, max_depth + 1)
        new_depth = min(new_depth, real_depth)    
        if new_depth == 1:
            indices = np.array([0])
        else:
            indices = np.random.choice(real_depth - 1, 
                    size=[new_depth - 1], replace=False)
            indices = np.append(0, indices + 1).astype('int')
        return indices

    msa_indices = _get_random_indices(
            len(raw_features['msa']), 
            min_depth, 
            max_depth)
    for k in ['msa', 'deletion_matrix_int', 'msa_species_identifiers']:
        raw_features[k] = raw_features[k][msa_indices]
    raw_features['num_alignments'][:] = len(msa_indices)

    if 'msa_all_seq' in raw_features:
        msa_all_seq_indices = _get_random_indices(
                len(raw_features['msa_all_seq']), 
                min_depth, 
                max_depth)
        for k in ['msa_all_seq', 'deletion_matrix_int_all_seq', 
                'msa_species_identifiers_all_seq']:
            raw_features[k] = raw_features[k][msa_all_seq_indices]
    return raw_features


def block_delete_msa(raw_features, config):
    """Sample MSA by deleting contiguous blocks.

    Jumper et al. (2021) Suppl. Alg. 1 "MSABlockDeletion"
    
    Args:
        raw_features: dict, containing 
    """
    num_seq = raw_features['msa'].shape[0]
    block_num_seq = int(np.floor(num_seq * config.msa_fraction_per_block))
    ...
    del_block_starts = np.random.randint(0, num_seq, size=nb)
    del_blocks = del_block_starts[:, None] + np.arange(block_num_seq)
    del_blocks = np.clip(del_blocks, 0, num_seq - 1)
    del_indices = np.unique(np.sort(del_blocks.flatten()))

    # Make sure we keep the original sequence
    keep_indices = np.setdiff1d(np.arange(1, num_seq), del_indices)
    keep_indices = np.concatenate(([0], keep_indices))

    for k in _MSA_FEATURE_NAMES:
        if k in raw_features:
            raw_features[k] = raw_features[k][keep_indices]
    return raw_features
...
def generate_rigidgroups(prot):
    ...
    rigidgroups = all_atom.atom37_to_frames(
        paddle.to_tensor(prot['aatype_index'][None], place=cpu, stop_gradient=True),          # (B, N)
        paddle.to_tensor(prot['all_atom_positions'][None], place=cpu, stop_gradient=True),    # (B, N, 37, 3)
        paddle.to_tensor(prot['all_atom_mask'][None], place=cpu, stop_gradient=True))         # (B, N, 37)
