...
from helixfold.data.pipeline_hybrid import AF2_PADDING_DIM, AF2_PADDING_FEATS, \
...
def _correct_post_merged_feats(
  ...
  if not pair_msa_sequences:
    ...
    ## TODO: bert_mask is not used anymore in AF3.
  ...
  for feature_name in all_merge_keys:
    ...
    if feature_name_split in MSA_FEATURES:
      if pair_msa_sequences or '_all_seq' in feature_name:
        ...
        ## NOTE: AF3 use the dense_pad function instead of block_diag for MSA_FEATURES.
        merged_example[feature_name] = dense_pad(
              *feats, padding_dim=AF2_PADDING_DIM[feature_name_split],
...
def merge_chain_features(np_chains_list: List[pipeline.FeatureDict],
  ...
  # AF3 change: paired and unpaired MSA features will be always concatenated in dense manner.
  # It is different from AF2, where unpaired MSA features were always block-diagonalised; paired MSA
  # features were concatenated.
  np_example = _merge_features_from_multiple_chains(
      np_chains_list, pair_msa_sequences=False) # in AF2, pair_msa_sequences=False
