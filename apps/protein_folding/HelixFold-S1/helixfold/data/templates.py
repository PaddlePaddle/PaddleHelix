...
def _find_template_in_pdb(
    ...
    exact_match=False) -> Tuple[str, str, int]:
  ...
  Args:
    ...
    exact_match: If True, this function will only match the template_chain_id sequence.
  ...
  """
  if exact_match:
    print(f'only match the {template_chain_id} sequence.')
    ...
    for chain_id, chain_sequence in mmcif_object.chain_to_seqres.items():
      if os.environ.get('DEBUG', ""):
        logging.info(f"chain_id: {chain_id}, chain_sequence: {chain_sequence}")
      if chain_sequence and (template_sequence in chain_sequence):
...
def _realign_pdb_template_to_query(
  ...
  seq_ident = float(num_same) / min(
      len(old_template_sequence), len(new_template_sequence))
  logging.info(f'seq_ident: {seq_ident}')
...
def _extract_template_features(
    ...
    ref_structure=False) -> Tuple[Dict[str, Any], Optional[str]]:
  ...
  try:
    if ref_structure:
      ...
      seqres, chain_id, mapping_offset = _find_template_in_pdb(
          ...
          mmcif_object=mmcif_object)
    if os.environ.get('DEBUG', ""):
        logging.info(f"seqres: {seqres}, chain_id: {chain_id}, mapping_offset: {mapping_offset}")
...
def _process_single_hit(
  ...
  if os.path.exists(cif_path):
    ...
    return SingleHitResult(features=None, error='No valid cif', warning=None)
...
def _run_multi_hits(tasks, _max_hits):
    """
    运行多个任务，返回最大命中数的结果。
    
    Args:
        tasks (list[tuple]): 包含三个元素的元组列表，分别是hit对象、hit的名称和hit的命中数。
        _max_hits (int): 最大命中数，超过此数量的命中将被跳过。
    
    Returns:
        list[Result]: Result类型的列表，包含所有命中数小于等于_max_hits的结果。每个结果都包含一个特征向量，如果特征向量为None，则说明该命中无效，并记录错误和警告信息。
    """
    res = []
    already_seen = set()
    for task in tasks:
        hit = task[2]
        result = _process_single_hit(*task)
        if len(already_seen) >= _max_hits:
            break
...
class HmmsearchHitFeaturizer(TemplateHitFeaturizer):
  ...
  def get_templates_block_parallel(
      self,
      query_sequence: str,
      query_pdb_code: Optional[str],
      query_release_date: Optional[datetime.datetime],
      hits: Sequence[parsers.TemplateHit],
      min_hits_for_task: int=5) -> TemplateSearchResult:
    """Computes the templates for given query sequence (more details above)."""
    ...
    tasks_block, tasks = [], []
    i_task = 0
    #for hit in sorted_hits[:min(len(sorted_hits), 30000)]:
    for hit in sorted_hits:
      if (i_task + 1) % 1000 == 0:
        tasks_block.append(tasks)
        tasks = []
      tasks.append((query_sequence, query_pdb_code, hit, self._mmcif_dir, template_cutoff_date, \
                  self._release_dates, self._obsolete_pdbs, self._strict_error_check, self._kalign_binary_path))
      i_task += 1
    logging.info('[_process_block_parrallel: %d block.', len(tasks_block))
    logging.info('[_process_single_hit tasks num: %d', len(sorted_hits))
    with ProcessPoolExecutor(max_workers=8) as executor:
      future_to_hit = {executor.submit(_run_multi_hits, tasks, max(self._max_hits // len(tasks_block), min_hits_for_task)): \
                      (index, tasks) for index, tasks in enumerate(tasks_block)}
      results_list = []
      for futures_ in as_completed(future_to_hit):
          index, tasks_ = future_to_hit[futures_]
          try:
              result_tasks = futures_.result()
          except Exception as exc:
              print(f"Task {index} generated an exception: {exc}")
              continue
          results_list.append((index, result_tasks, tasks_))

      results_list.sort(key=lambda x: x[0]) 

      for _, result_tasks, tasks_ in results_list:
          for result, task in zip(result_tasks, tasks_):
              hit = task[2]
  ...
  def get_templates_parallel(
    ...
    for hit in sorted_hits:
      tasks.append((query_sequence, query_pdb_code, hit, self._mmcif_dir, template_cutoff_date, \
                  self._release_dates, self._obsolete_pdbs, self._strict_error_check, self._kalign_binary_path))
    tasks = tasks[:min(len(tasks), 1000)]
    logging.info('[_process_single_hit tasks num: %d', len(sorted_hits))
    with ThreadPoolExecutor(max_workers=10) as executor:
      future_to_hit = {executor.submit(_process_single_hit, *task): task for task in tasks}

      for future in as_completed(future_to_hit):
        hit = future_to_hit[future][2]
        ...
        try:
            ...
            logging.error(f"Error processing hit {hit.name}: {str(e)}")
            continue
    ...
    return TemplateSearchResult(
        features=template_features, errors=errors, warnings=warnings)
  def run_multi_hits(self, tasks):
    """ run multi hits function. """
    res = []
    for task in tasks:
      result = _process_single_hit(*task)
      res.append(result)
  ...
  def get_templates(
    ...
    if not hits or hits[0].sum_probs is None:
      ...
      sorted_hits = sorted(hits, key=lambda x: x.sum_probs, reverse=True)
    # add max hits limit :1000
    for hit in sorted_hits[:min(len(sorted_hits), 1000)]:
      # We got all the templates we wanted, stop processing hits.
      if len(already_seen) >= self._max_hits:
        break
