...
import os
import tempfile
from typing import Mapping, MutableMapping, Sequence, List, Dict, Tuple, Optional
...
from helixfold.data import msa_pairing_rna
...
class RNADataPipeline:
  ...
  def _all_seq_msa_features(self, msa_file, query_sequence):
    ...
    all_seq_features = pipeline_rna.make_msa_features([msa], species_identifer_df=species_identifer_df, max_align_depth=max_align_depth)
    valid_feats = msa_pairing_rna.MSA_FEATURES + (
...
def process_with_all_chain_features(
  ...
  for chain_id, chain_features in all_chain_features.items():
    # process chain_features to map to new token list
    chain_features = process_feat_to_mapping_to_new_token_list(chain_features)
...
def process_feat_to_mapping_to_new_token_list(chain_features):
    ...
    chain_features['aatype'] = transform_onehot_features(chain_features['aatype'],
                                          ...
                                          new_token_id_map=residue_constants.AF3_restype_order,
                                          unmatched_token='N')
