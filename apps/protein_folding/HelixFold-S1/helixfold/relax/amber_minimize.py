...
from helixfold.model import folding
# from helixfold.relax import cleanup
# from helixfold.relax import utils
...
def between_residue_bond_loss_np(
  ...
  c_n_bond_length = np.sqrt(
      1e-6 + np.sum((this_c_pos - next_n_pos) ** 2, axis=-1))
  ...
  c_n_loss_per_residue = np.maximum(
      c_n_bond_length_error - tolerance_factor_soft * gt_stddev, 0)
  ...
  ca_c_bond_length = np.sqrt(1e-6 + np.sum(
      (this_ca_pos - this_c_pos) ** 2, axis=-1))
  n_ca_bond_length = np.sqrt(1e-6 + np.sum(
      (next_n_pos - next_ca_pos) ** 2, axis=-1))
  ...
  ca_c_n_loss_per_residue = np.maximum(
      ca_c_n_cos_angle_error - tolerance_factor_soft * gt_stddev, 0)
  ...
  c_n_ca_loss_per_residue = np.maximum(
      c_n_ca_cos_angle_error - tolerance_factor_soft * gt_stddev, 0)
...
def between_residue_clash_loss_np(
  ...
  dists = np.sqrt(1e-10 + np.sum(
      (atom14_pred_positions[:, None, :, None, :] -
       atom14_pred_positions[None, :, None, :, :]) ** 2,
      axis=-1))
  ...
  c_one_hot = np.eye(14)[2]
  n_one_hot = np.eye(14)[0]
  ...
  # Compute the error.
  # shape (N, N, 14, 14)
  dists_to_low_error = dists_mask * np.maximum(
      dists_lower_bound - overlap_tolerance_soft - dists, 0)
...
def within_residue_violations_np(
    atom14_pred_positions: np.ndarray,  # (N, 14, 3)
    atom14_atom_exists: np.ndarray,  # (N, 14)
  ...
  dists = np.sqrt(1e-10 + np.sum(
      (atom14_pred_positions[:, :, None, :] -
       atom14_pred_positions[:, None, :, :]) ** 2,
      axis=-1))

  # Compute the loss.
  # shape (N, 14, 14)
  dists_to_low_error = np.maximum(
      atom14_dists_lower_bound + tighten_bounds_for_loss - dists, 0)
  dists_to_high_error = np.maximum(
      dists - (atom14_dists_upper_bound - tighten_bounds_for_loss), 0)
...
def find_structural_violations_np(
  ...
  connection_violations = between_residue_bond_loss_np(
  ...
  atom14_atom_radius = batch['atom14_atom_exists'] * np.take(
      atomtype_radius, batch['residx_atom14_to_atom37'])

  # Compute the between residue clash loss.
  between_residue_clashes = between_residue_clash_loss_np(
  ...
  atom14_dists_lower_bound = np.take(
      restype_atom14_bounds['lower_bound'], batch['aatype'], axis=0)
  atom14_dists_upper_bound = np.take(
      restype_atom14_bounds['upper_bound'], batch['aatype'], axis=0)
  within_residue_violations = within_residue_violations_np(
      atom14_pred_positions=atom14_pred_positions,
      atom14_atom_exists=batch['atom14_atom_exists'],
...
def extreme_ca_ca_distance_violations_np(
    pred_atom_positions: np.ndarray,  # (N, 37(14), 3)
    pred_atom_mask: np.ndarray,  # (N, 37(14))
    residue_index: np.ndarray,  # (N)
  ...
  ca_ca_distance = np.sqrt(
      1e-6 + np.sum((this_ca_pos - next_ca_pos) ** 2, axis=-1))
  violations = (ca_ca_distance -
                residue_constants.ca_ca) > max_angstrom_tolerance
  mask = this_ca_mask * next_ca_mask * has_no_gap_mask
  return utils.mask_mean_np(mask=mask, value=violations)
...
def compute_violation_metrics_np(
  ...
  ret = {}
  extreme_ca_ca_violations = extreme_ca_ca_distance_violations_np(
      pred_atom_positions=atom14_pred_positions,
      pred_atom_mask=batch['atom14_atom_exists'].astype(np.float32),
      residue_index=batch['residue_index'].astype(np.float32))
  ret['violations_extreme_ca_ca_distance'] = extreme_ca_ca_violations
  ret['violations_between_residue_bond'] = utils.mask_mean_np(
      mask=batch['seq_mask'],
      value=violations['between_residues'][
          'connections_per_residue_violation_mask'])
  ret['violations_between_residue_clash'] = utils.mask_mean_np(
      mask=batch['seq_mask'],
      value=np.max(
          violations['between_residues']['clashes_per_atom_clash_mask'],
          axis=-1))
  ret['violations_within_residue'] = utils.mask_mean_np(
      mask=batch['seq_mask'],
      value=np.max(
          violations['within_residues']['per_atom_violations'], axis=-1))
  ret['violations_per_residue'] = utils.mask_mean_np(
      mask=batch['seq_mask'],
