...
from helixfold.model import folding, lddt, quat_affine, all_atom, geometry
...
def softmax_cross_entropy(logits, labels):
    ...
    if os.environ.get('DCU_MODE_SOFTMAX_CLIP', '0') == '1':
        # clip scaling 
        logits_mean = logits.mean(-1,keepdim=True)
        logits -= logits_mean
        limit = 50
        logits = paddle.clip(logits, min=-1*limit, max=limit)
...
class HelixFold(nn.Layer):
    """HelixFold model with recycling.
...
class MSARowAttentionWithPairBias(nn.Layer):
    ...
    def forward(self, msa_act, msa_mask, pair_act):
        ...
        if not self.training or (self.is_extra_msa and self.config.get("use_subbatch",False)):
...
class Transition(nn.Layer):
    ...
    def forward(self, act, mask):
        ...
        if not self.training:
            ...
            sb_transition = subbatch(transition_module, [0], [1],
                                 self.global_config.train_subbatch_size, 1)
            act = sb_transition(act)           
        else:
...
class PredictedAlignedErrorHead(nn.Layer):
    ...
    def loss(self, value, batch):
        ...
        if self.global_config.multimer_mode:
            predicted_affine = geometry.Rigid3Array.from_array(
                value['structure_module']['final_affines'])
            quat = quat_affine.rot_to_quat(predicted_affine.rotation.to_array())
            trans = predicted_affine.translation.to_array()
            predicted_affine = quat_affine.QuatAffine(quat, trans)
        else:
    ...
    def forward(self, msa_act, pair_act, masks):
        if self.global_config.get('multimer_mode', False):
            if self.config.outer_product_mean.first:
                self.global_config.outer_product_mean_position = 'first'
        ...
        if self.global_config.outer_product_mean_position in ['origin', 'middle']:
            ...
            raise ValueError("Only support outer_product_mean_position in ['origin', 'middle', ''first', 'end'] now!")
...
class TriangleMultiplication(nn.Layer):
    ...
    def __init__(self, channel_num, config, global_config, name='triangle_multiplication'):
        ...
        if self.config.get("fuse_projection_weights", False):
            # TODO(zhukunrui): check if LayerNorm init aligns with _layer_norm from af2multimer v3
