...
def multi_chain_permutation_align(batch, label, out):
    ...
    label = tree_map_with_list(lambda x: x.numpy(), label)
    out = tree_map(lambda x: x.numpy(), out)
...
def chain_align_per_sample(batch_feat, batch_label, out, batch_i):
    ...
    for i in batch_label.keys():
        if i == 'resolution':
            ...
            pad_shape = [N_res - new_label.shape[0]] + list(new_label.shape[1:])
            pad_value = np.zeros(pad_shape, new_label.dtype)
            aligned_label[i] = np.concatenate([new_label, pad_value], 0)
        
    # TODO:
    # assert np.mean(batch_feat['aatype'][batch_i] == aligned_label['aatype_index']) > 0.9, \
