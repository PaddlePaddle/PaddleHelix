...
def atom37_to_torsion_angles(
    ...
    aatype = paddle.minimum(
        aatype.astype('int'), paddle.full(shape=[1], fill_value=20, dtype="int64"))  
...
def frame_aligned_point_error(
    ...
    length_scale: float,
    pair_mask: paddle.Tensor = None,
    l1_clamp_distance: Optional[float] = None,
    epsilon=1e-4,
    return_coverage=False) -> paddle.Tensor:
    ...
    if pair_mask is not None:
        ...
        mask *= pair_mask
        normalization_factor = mask.sum((-1, -2))
    else:
        normalization_factor = (
            paddle.sum(frames_mask, axis=-1) *
            paddle.sum(positions_mask, axis=-1))
            
    fape = (paddle.sum(normed_error, axis=[-2, -1]) /
            (epsilon + normalization_factor))

    if return_coverage:
        if l1_clamp_distance:
            flag = paddle.logical_and(normed_error > 0, 
                    normed_error < l1_clamp_distance / length_scale).astype('float32')
            coverage = paddle.sum(flag) / (paddle.sum(normalization_factor) + epsilon)
        else:
            coverage = paddle.zeros([1], 'float32')
        return fape, coverage
    return fape