...
from helixfold.model import quat_affine, geometry, modules
from helixfold.model import r3
from helixfold.model import utils
from helixfold.model import folding_multimer
from helixfold.model.geometry import vec_rot
from helixfold.model.chain_align import get_transform

class ChainAffine(nn.Layer):
    """
    chain level affine
    """
    def __init__(self, num_channel, config, global_config):
        super(ChainAffine, self).__init__()
        self.config = config
        ...
        self.act_ln = nn.LayerNorm(num_channel)
        self.in_mlp = nn.Sequential(
            nn.ReLU(),
            Linear(num_channel, c),
        )
        self.res_blocks = nn.LayerList()
        for i in range(2):
            res_block = self.mlp = nn.Sequential(
                nn.ReLU(),
                Linear(c, c),
                nn.ReLU(),
                Linear(c, c),
    ...
    def _forward_resblock(self, act):
        out_act = self.act_ln(act)
        out_act = self.in_mlp(out_act)
        for block in self.res_blocks:
            out_act += block(out_act)
        out_act = self.out_mlp(out_act)
        return out_act
    
    def forward(self, act, asym_id, use_new_affine):
        """
        act: (b, num_res, d)
        asym_id: (b, num_res)
        """
        batch_size, num_res, hidden_size = act.shape[0:3]
        assert batch_size == 1, "fix 1-D tensor warning for batch>1"
        out_act_list = []
        for batch_i in range(batch_size):
            out_act = paddle.zeros([num_res, 6], act.dtype)

            cur_asym_id = asym_id[batch_i]
            valid_asym_id = cur_asym_id[cur_asym_id > 0]
            uniq_asym_id = [int(x) for x in paddle.unique(valid_asym_id)]
            for aid in uniq_asym_id:
                index = paddle.where(cur_asym_id == aid)[0].flatten()
                chain_act = paddle.mean(
                    paddle.reshape(act[batch_i][index], (-1, hidden_size)), # (sub_num_res, d) avoid shape mismatch when sub_num_res==1
                    axis=0, keepdim=True)   # (1, d)
                chain_act = self._forward_resblock(chain_act).tile([index.shape[0], 1])  # (sub_num_res, 6)
                tmp_act = paddle.zeros_like(out_act)
                out_act += paddle.scatter(tmp_act, index, chain_act, overwrite=True)    # (num_res, 6)
            out_act_list.append(out_act)

        out_act_list = paddle.stack(out_act_list)   # (b, num_res, 6)

        if use_new_affine:
            qx, qy, qz = paddle.unstack(out_act_list[..., :3], axis=-1)
            qw = paddle.ones_like(qx)
            rotation = geometry.Rot3Array.from_quaternion(
                qw, qx, qy, qz, normalize=True)
            translation = geometry.Vec3Array(*paddle.unstack(out_act_list[..., 3:], axis=-1))
            chain_affine = geometry.Rigid3Array(rotation, translation)
        else:
            ones = paddle.ones([batch_size, num_res, 1], out_act_list.dtype)
            out_act_list = paddle.concat([ones, out_act_list], -1)      # (b, num_res, 7)
            chain_affine = quat_affine.QuatAffine.from_tensor(out_act_list)
        return chain_affine
...
class FoldIteration(nn.Layer):
    ...
    def __init__(self, channel_num, config, global_config):
        ...
        if self.global_config.get('multimer_mode', False):
            self.invariant_point_attention = \
                folding_multimer.InvariantPointAttention(
        ...
        if self.config.get('use_chain_affine', False):
            self.chain_affine_update = ChainAffine(
                    self.config.num_channel, self.config.chain_affine, self.global_config)
                    
        if self.config.get('use_chain_affine_v2', False):
            self.chain_affine_update = ChainAffine(
                    self.config.num_channel, self.config.chain_affine, self.global_config)
    ...
    def forward(self, activations, init_single_act, static_pair_act,
                seq_mask, aatype, asym_id):
        """
        tbd
        """
        use_new_affine = isinstance(activations['affine'], geometry.Rigid3Array)
        if use_new_affine:
        ...
        attn = modules.recompute_wrapper(
            self.invariant_point_attention,
            act, static_pair_act, seq_mask, affine.to_array(),
            is_recompute=True,
            )
        act += attn
        ...
        if use_new_affine:
            qx, qy, qz = paddle.unstack(affine_update[..., :3], axis=-1)
            qw = paddle.ones_like(qx)
            rotation = geometry.Rot3Array.from_quaternion(
                qw, qx, qy, qz, normalize=True)
            translation = geometry.Vec3Array(*paddle.unstack(affine_update[..., 3:], axis=-1))
            affine_update = geometry.Rigid3Array(rotation, translation)
            affine = affine @ affine_update
        else:
            affine = affine.pre_compose(affine_update)

        if self.config.get('use_chain_affine', False):
            chain_affine = self.chain_affine_update(act, asym_id, use_new_affine)
            if use_new_affine:
                affine = chain_affine @ affine
            else:
                affine = chain_affine.pre_compose_affine(affine)

        if self.config.get('use_chain_affine_v2', False):
            chain_affine = self.chain_affine_update(act, asym_id, use_new_affine)
            if "last_chain_affine" in activations:
                chain_affine = activations["last_chain_affine"] @ chain_affine if use_new_affine else activations["last_chain_affine"].pre_compose_affine(chain_affine)
            affine = chain_affine @ affine if use_new_affine else chain_affine.pre_compose_affine(affine)
        sc = self.rigid_sidechain(
            affine.scale_translation(self.config.position_scale),
            act, init_single_act, aatype)

        if use_new_affine:
            outputs = {'affine': affine.to_array(), 'sc': sc}
        else:
        ...
        new_activations = {
            'act': act,
            'affine': affine if use_new_affine else affine.to_tensor()
        }
        if self.config.get('use_chain_affine_v2', False): new_activations.update({"last_chain_affine": chain_affine})
...
class StructureModule(nn.Layer):
    ...
    def forward(self, representations, batch):
        ...
        if self.global_config.multimer_mode:
            # NOTE: use new affine for multimer by default
            affine = geometry.Rigid3Array.from_array(output['affine'])
            ret['traj'] = affine.scale_translation(self.config.position_scale).to_array()
        else:
    ...
    def loss(self, value, batch):
        def _count_is_single(batch):
            is_single_list = []
            for asym_id in batch['asym_id']:
                if paddle.unique(asym_id[asym_id > 0]).shape[0] == 1:
                    is_single_list.append(1)
                else:
                    is_single_list.append(0)
            return paddle.to_tensor(is_single_list, 'float32')

        ret = {'loss': 0.}

        ret['metrics'] = {}
        # If requested, compute in-graph metrics.
        if self.config.get('compute_in_graph_metrics', True):
        ...
        multimer_mode = self.global_config.get('multimer_mode', False)
        if not multimer_mode:
            bb_loss, fape, _ = backbone_loss(
                ret, batch, value, self.config, None, multimer_mode=False)
            ret['fape'] = fape
            ret['backbone_fape'] = bb_loss
            ret['loss'] += bb_loss
        else:
            intra_chain_mask = batch['asym_id'][:, :, None] == \
                batch['asym_id'][:, None, :]
            intra_chain_bb_loss, intra_chain_fape, intra_fape_coverage = backbone_loss(
                ret, batch, value, self.config.intra_chain_fape,
                intra_chain_mask, multimer_mode=True)
            interface_bb_loss, interface_fape, interface_fape_coverage = backbone_loss(
                ret, batch, value, self.config.interface_fape,
                1. - intra_chain_mask, multimer_mode=True)
            bb_loss = intra_chain_bb_loss + interface_bb_loss
            ret['intra_chain_fape'] = intra_chain_fape
            ret['intra_fape_coverage'] = intra_fape_coverage
            ret['interface_fape'] = interface_fape
            ret['interface_fape_coverage'] = interface_fape_coverage
            ret['interface_fape_is_single'] = _count_is_single(batch).mean()
            ret['fape'] = intra_chain_fape + interface_fape
            ret['backbone_fape'] = bb_loss
            ret['loss'] += bb_loss

            ## chain center-of-mass loss
            cm_loss, extra_info = chain_center_of_mass_loss(batch, value, self.config)
            ret['center_of_mass_loss'] = cm_loss
            # use "fakeloss" in order to be captured by tensorboard
            ret.update({'fakeloss_' + k: v for k, v in extra_info.items()})
            ret['loss'] += cm_loss
    ...
    def _generate_affines(self, representations, batch):
        ...
        use_new_affine = isinstance(
            self.fold_iteration.invariant_point_attention,
            folding_multimer.InvariantPointAttention)

        if use_new_affine:
            affine = geometry.Rigid3Array.identity(shape=seq_mask.shape[:-1])
        else:
            affine = generate_new_affine(seq_mask).to_tensor()

        activations = {'act': single_act, 'affine': affine}
        ...
        for _ in range(self.config.num_layer):
            activations, output = self.fold_iteration(
                ...
                seq_mask, batch['aatype'], batch['asym_id'])
...
def chain_center_of_mass_loss(batch, value, config, eps=1e-6):
    """
    chain center-of-mass loss such that chains are 
    pushed apart if they are closer than in the ground 
    truth
    Args:
        batch: contains ground-truth labels
        value: contains model predictions
    """
    def _center_distance(centers):
        ...
        return:
            dist: (M, M)
        """
        ci = centers.unsqueeze([1])
        cj = centers.unsqueeze([0])
        dist = ci - cj  # (M, M, 3)
        dist = paddle.sqrt(paddle.sum(dist ** 2, -1) + eps)   # (M, M)
        return dist
    def _statistic_dist(dist):
        """
        Args:
            dist: (M, M)
        """
        amin = amax = amean = paddle.to_tensor(0.0)
        M = dist.shape[0]
        if M > 1:
            vs = dist[paddle.eye(M) == 0]
            amin = paddle.min(vs)
            amax = paddle.max(vs)
            amean = paddle.mean(vs)
        return amin, amax, amean

    center_of_mass_weight = config.get('center_of_mass_weight', 0)

    total_loss = []
    extra_info = {
        'chain_num': [], 
        'gt_dist_min': [], 'gt_dist_max': [], 'gt_dist_mean': [],
        'pred_dist_min': [], 'pred_dist_max': [], 'pred_dist_mean': []}
    ca_idx = residue_constants.atom_order['CA']
    batch_size = batch['all_atom_positions'].shape[0]
    for batch_i in range(batch_size):
        gt_asym_id = batch['asym_id'][batch_i]      # (N)
        gt_ca_mask = batch['all_atom_mask'][batch_i, :, ca_idx] # (N)
        seq_mask = paddle.logical_and(gt_asym_id > 0, gt_ca_mask == 1)
        gt_asym_id = gt_asym_id[seq_mask]       # (n)
        gt_ca_pos = batch['all_atom_positions'][batch_i, :, ca_idx][seq_mask]       # (n, 3)
        pred_ca_pos = value['final_atom_positions'][batch_i, :, ca_idx][seq_mask]   # (n, 3)

        ## cal centers
        gt_center_list = []
        pred_center_list = []
        uniq_asym_id = [int(x) for x in paddle.unique(gt_asym_id)]  # (M)
        for aid in uniq_asym_id:
            aid_mask = gt_asym_id == aid
            gt_center_list.append(paddle.mean(gt_ca_pos[aid_mask], 0))     # (3)
            pred_center_list.append(paddle.mean(pred_ca_pos[aid_mask], 0)) # (3)
        gt_center_list = paddle.stack(gt_center_list)       # (M, 3)
        pred_center_list = paddle.stack(pred_center_list)   # (M, 3)

        ## cal loss
        gt_dist = _center_distance(gt_center_list)      # (M, M)
        pred_dist = _center_distance(pred_center_list)  # (M, M)
        if config.center_of_mass_loss_type == 'l1':
            loss = - paddle.clip(pred_dist - gt_dist + 4, max=0)
        elif config.center_of_mass_loss_type == '1to1':
            loss = nn.functional.smooth_l1_loss(pred_dist, gt_dist, reduction='none')
        elif config.center_of_mass_loss_type == 'min':
            loss = paddle.clip(paddle.clip(gt_dist, max=4) - pred_dist, min=0)
        else:
            loss = paddle.clip((pred_dist - gt_dist + 4) / 20, max=0) ** 2
        total_loss.append(paddle.mean(loss) * center_of_mass_weight * \
                batch['is_spatial_crop'].squeeze(-1)[batch_i])

        ## statitis of dist
        extra_info['chain_num'].append(paddle.to_tensor(gt_dist.shape[0], 'float32'))
        amin, amax, amean = _statistic_dist(gt_dist)
        extra_info['gt_dist_min'].append(amin)
        extra_info['gt_dist_max'].append(amax)
        extra_info['gt_dist_mean'].append(amean)
        amin, amax, amean = _statistic_dist(pred_dist)
        extra_info['pred_dist_min'].append(amin)
        extra_info['pred_dist_max'].append(amax)
        extra_info['pred_dist_mean'].append(amean)
    total_loss = paddle.stack(total_loss)
    extra_info = {k: paddle.stack(v) for k, v in extra_info.items()}
    return total_loss, extra_info


def backbone_loss(ret, batch, value, config, pair_mask, multimer_mode=False):
    """Backbone FAPE Loss.
    ...
    Args:
        ...
        config: Configuration of loss, when multimer_mode is False,
          should contain 'fape.clamp_distance' and 'fape.loss_unit_distance';
          when multimer_mode is True, should contain 'atom_clamp_distance' and
          'loss_unit_distance'.
        pair_mask: A mask used for separating intra from inter chain losses.
        multimer_mode: Whether this loss function is for AF2-Multimer.
    """
    # TODO: upgrade to use new affine
    if multimer_mode:
        affine_trajectory = geometry.Rigid3Array.from_array(value['traj'])
        rot = r3.Rots(affine_trajectory.rotation.to_array())
        vec = r3.Vecs(affine_trajectory.translation.to_array())
        rigid_trajectory = r3.Rigids(rot, vec)
    else:
    ...
    for rigid_trajectory_rot_item,rigid_trajectory_trans_item in zip(rigid_trajectory.rot,rigid_trajectory.trans):
        ...
        index+=1
        middle_fape_loss, coverage = fape_loss_fn(rigid_trajectory_item, gt_rigid, backbone_mask,
                           rigid_trajectory_trans_item, gt_rigid.trans,
                           backbone_mask)
        fape_loss.append(middle_fape_loss)
        coverage_list.append(coverage)
    fape_loss = paddle.stack(fape_loss)
    coverage_list = paddle.stack(coverage_list)

    if 'use_clamped_fape' in batch and not multimer_mode:
    ...
    return paddle.mean(fape_loss), fape_loss[-1], coverage_list[-1]


def sidechain_loss(batch, value, config, multimer_mode=False):
...
def structural_violation_loss(ret, batch, value, config):
    ...
    ret['loss'] += (config.structural_violation_loss_weight * (
        ...
        paddle.sum(
            violations['between_residues']['clashes_per_atom_loss_mean'] +
...
def find_structural_violations(
    ...
    return {
        'between_residues': {
            ...
            'clashes_per_atom_loss_mean':
                between_residue_clashes['per_atom_loss_mean'],  # (B, N, 14)
...
def supervised_chi_loss(ret, batch, value, config):
    ...
    chi_pi_periodic = paddle.einsum('nijk,kl->nijl', residue_type_one_hot.unsqueeze(axis=1),
                            paddle.to_tensor(residue_constants.chi_pi_periodic))
...
class MultiRigidSidechain(nn.Layer):
    ...
    def forward(self, affine, single_act, init_single_act, aatype):
        ...
        use_new_affine = isinstance(affine, geometry.Rigid3Array)

        # TODO: update to use new affine in all_atom.* functions
