...
def prediction_to_pdb(pred_atom_pos: Union[np.ndarray, paddle.Tensor], 
  ...
  restypes_mapping = residue_constants.AF3_restype_order
  ...
  if np.any(aatype >= residue_constants.AF3_restype_nums):
...
if __name__ == '__main__':
  ...
  demo_feats_pkl = './helixfold/common/testdata/7z1k/features.pkl'
  demo_label_pkl = './helixfold/common/testdata/7z1k/label.pkl'

  demo_feats = _read_pickle(demo_feats_pkl)
  demo_label = _read_pickle(demo_label_pkl)
  import pdb;pdb.set_trace()
  demo_feats.update(demo_label)

  pdb_file_path = './helixfold/common/testdata/7z1k/test_7z1k_save_check.pdb'
  pred_atom_pos = demo_label['all_atom_pos'] ## use the groundTruth to check code.
  t = prediction_to_pdb(pred_atom_pos, demo_feats, pdb_file_path)
  print(t)

  ## prediction to mmcif
  ## NOTE: becareful, maxit_binary should be installed.
  ## links: https://sw-tools.rcsb.org/apps/MAXIT/source.html
  ## RCSBROOT=/root/paddlejob/workspace/output/yexianbin/dcu_command/maxit-v11.200-prod-src
  ## export RCSBROOT
  ## PATH="$RCSBROOT/bin:"$PATH; export PATH
  maxit_binary = '/root/paddlejob/workspace/output/yexianbin/dcu_command/maxit-v11.200-prod-src/bin/maxit'
  cif_file_path = './alphafold_paddle/common/testdata/7z1k/test_7z1k_save_check.cif'
  t = prediction_to_mmcif(pred_atom_pos, demo_feats, maxit_binary, cif_file_path)
  print(t)