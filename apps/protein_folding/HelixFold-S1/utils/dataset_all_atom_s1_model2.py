...
from utils.utils import download_interface_prob_files, find_max_numeric_directory
...
class AllAtomDataset(paddle.io.Dataset):
    ...
    def __init__(self,
        ...
        self.pocket_map = self.data_config.get('pocket_map',
                {'centra_10A': 1.0})
        self.interface_map = self.data_config.get('interface_map',
                {'rand_m_from_top_n': 1.0})
        ...
        self.use_interface_info = self.data_config.get("use_interface_info", False)
        self.interface_config = self.data_config.get("interface_config", 
            {"dist_thres": 5, "top_n": 40, "min_m": 1, "max_m": 20, "seed": None})
    ...
    def get_interface_info(self, feat, label):

        interface_type = random.choices(
                list(self.interface_map.keys()),
                weights=list(self.interface_map.values()),
                k=1)[0]

        if interface_type == 'any_interface':
            # random select m between min_m to max_m
            m = random.randint(self.interface_config['min_m'], self.interface_config['max_m'])
            seed = self.interface_config.get('seed', None)
            sample_times = self.interface_config.get('sample_times', 1)
            interface_info_list = build_interface_info_with_random_residues(
                    feat['asym_id'], 
                    ...
                    top_n=self.interface_config.get('top_n', -1),
                    m=m,
            ...
            interface_map_list = [interface_info.interface_map() for interface_info in interface_info_list]
            interface_info = np.stack(interface_map_list, axis=-1).astype("float32")
        elif interface_type == 'epitope_paratope':
            n_token = len(feat['asym_id'])
            token_mask = label['all_centra_token_indice_mask']
            pocket_info = np.zeros([n_token, n_token], np.float32)
            is_polymer = np.logical_and(feat['is_ligand'] != 1, token_mask)
            ...
            interface_info_list = build_interface_info_with_random_residues(
                    ...
                    top_n=self.interface_config.get('top_n', -1),
                    m=m,
                    binder_mask=is_antibody, target_mask=is_antigen,
                    dist_thres=self.interface_config['dist_thres'],
                    dist_type=self.interface_config.get('dist_type', 'heavy_atom'),
                    seed=seed,
                    sample_times=sample_times)
            ...
            n_token = len(feat['asym_id'])
            interface_info = np.zeros([n_token, n_token, 1], 'float32')
        else:
            raise ValueError(interface_type)

        print(f"[Debug] Interface Info Shape: {interface_info.shape}")
    ...
    def get_sample(self, protein, sampled_chain_ids=None, sampling_method=None):
        ...
        if self.use_interface_info:
            # build interface info
            feat['interface_info'] = self.get_interface_info(feat, labels)     
...
class AllAtomTestPosebusterDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.use_interface_info = self.data_config.get("use_interface_info", False)
        self.interface_config = self.data_config.get("interface_config", 
            {"dist_thres": 5, "top_n": 10, "min_m": 10, "max_m": 10, "seed": 1})
        self.interface_type = self.data_config.get("interface_type", "protein_ligand")
    ...
    def set_blacklist(self, blacklist):
        # Create a set for quick lookup of predicted_cases entries
        # Convert each skip_case to a standardized format with sorted chain IDs
        skip_set = set()
        for case in blacklist:
            protein_part, *chain_parts = case.split('_')
            sorted_chains = '_'.join(sorted(chain_parts))
            skip_set.add(f"{protein_part}_{sorted_chains}")

        self.blacklist = skip_set
        return

    def check_in_blacklist(self, protein_json_str):
        # Parse the JSON string into a dictionary
        info = json.loads(protein_json_str)
        protein_name = info["protein_name"]
        
        # Generate all possible case names with sorted chain IDs
        case_name_list = []
        chain_ids = [chain["id"] for chain in info["chains"]]
        
        # Generate combinations of chain IDs, sorted alphabetically
        for r in range(1, len(chain_ids) + 1):
            for combo in combinations(chain_ids, r):
                sorted_combo = '_'.join(sorted(combo))
                case_name = f"{protein_name}_{sorted_combo}"
                case_name_list.append(case_name)

        # Check if the current protein_info is in skip_cases
        if any(case_name in self.blacklist for case_name in case_name_list):
            return True

        return False

    def filter_out_predicted_cases(self, predicted_cases):
        # reload protein infos
        protein_infos_json, failed_proteins, failed_codes = self._get_protein_infos()
        ...
        # Define a result list to store filtered protein_infos
        filtered_infos = []
    
        # Iterate over each protein_info JSON string
        for json_str in protein_infos_json:
            # Parse the JSON string into a dictionary
            ...
            # Check if the current protein_info is in skip_cases
            if not any(case_name in skip_set for case_name in case_name_list):
                filtered_infos.append(json_str)

            self.protein_infos = filtered_infos
    ...
    def get_full_interface_from_gt_structure(self, protein, feat, label, chain_info_list):

        if self.interface_type == "protein_ligand":
        ...
        full_interface_info_list = build_interface_info_with_random_residues(
                    feat['asym_id'], 
                    atom_pos=label['all_atom_pos'], 
                    atom_mask=label['all_atom_pos_mask'],
                    atom_to_token_mapping=feat['ref_token2atom_idx'], 
                    ...
                    top_n=-1,
                    m=-1,
                    binder_mask=binder_mask, target_mask=target_mask,
                    dist_thres=self.interface_config['dist_thres'],
                    dist_type=self.interface_config.get('dist_type', 'heavy_atom'),
                    seed=None)
        full_interface_info = full_interface_info_list[0]
        full_interface_info = full_interface_info.interface_map()
                
        return full_interface_info


    def get_interface_info(self, protein, feat, label, chain_info_list):

        targeted_chains = [chain["id"] for chain in chain_info_list]
        
        if self.interface_type in ["any_interface", "protein_ligand"]:
            binder_mask = feat['is_ligand'] if self.interface_type == "protein_ligand" else None
            target_mask = None
            ...
            interface_info_list = build_interface_info_with_random_residues(
                    ...
                    token_pos=label['all_atom_pos'][label['all_centra_token_indice']],
                    token_pos_mask=label['all_centra_token_indice_mask'],
                    top_n=self.interface_config['top_n'],
                    m=m,
                    binder_mask=binder_mask, target_mask=target_mask,
                    dist_thres=self.interface_config['dist_thres'],
                    dist_type=self.interface_config.get('dist_type', 'heavy_atom'),
            ...
            interface_map_list = [interface_info.interface_map() for interface_info in interface_info_list]
            interface_info = np.stack(interface_map_list, axis=-1).astype("float32")
        elif self.interface_type == 'generated':
            # get the file path
            interface_dir = self.data_config['interface_dir']
            n_token = len(feat['asym_id'])

            interface_indices_json_path = os.path.join(interface_dir, f"{protein}_{'_'.join(targeted_chains)}-gen-interfaces.json")
            if not os.path.exists(interface_indices_json_path):
                print(f'Not found Interface json file: {interface_indices_json_path}')
                interface_indices_pkl_path = os.path.join(interface_dir, f"{protein}_{'_'.join(targeted_chains)}-gen-interface-indices.pkl")
                if not os.path.exists(interface_indices_pkl_path):
                    print(f'Not found Interface pkl file: {interface_indices_pkl_path}')
                    interface_info = f'[{InterfaceInfo(n_token, probability=0.0).to_json()}]'    # empty interface info
                else:
                    # load from file
                    with open(interface_indices_pkl_path, 'rb') as pkl:
                        interface_info_indices = pickle.load(pkl)
                    if len(interface_info_indices) == 0:
                        print(f'Interface pkl file is empty: {interface_indices_pkl_path}')
                        interface_info = f'[{InterfaceInfo(n_token, probability=0.0).to_json()}]'    # empty interface info
                    else:
                        r, k, _ = interface_info_indices.shape # repeats(r), interface_size(k), indices(2)

                        m = random.randint(self.interface_config['min_m'], self.interface_config['max_m'])
                        interface_list = convert_interface_indices_to_interface_objs(interface_info_indices, n_token, max_nodes=m) 
                        dict_list = [obj.to_dict() for obj in interface_list]
                        interface_info = json.dump(dict_list, indent=4) 
                        # save interface info to json
                        with open(interface_indices_json_path, 'w') as f:
                            f.write(interface_info)
             
            else:
                with open(interface_indices_json_path, 'r', encoding='utf-8') as file:
                    try:
                        interface_info = ''.join(file.readlines())
                    except json.JSONDecodeError as e:
                        raise ValueError(f"Unable to parse JSON string: {e}")

        elif self.interface_type == "none":
            n_token = len(feat['asym_id'])
            interface_info = InterfaceInfo(n_token, probability=0.0).interface_map() # empty interface info
        else:
            raise ValueError(self.interface_type)

        return interface_info

    
    def get_interface_prob(self, protein, feat, chain_info_list):

        # get interface predicition model ckpt number
        interface_prob_ckpt_num = self.data_config.get('interface_prob_ckpt_num', None)

        if interface_prob_ckpt_num is None:
            # get the file path
            interface_dir = find_max_numeric_directory(self.data_config['local_interface_prob_dir'])
        else:
            interface_dir = os.path.join(self.data_config['local_interface_prob_dir'], str(interface_prob_ckpt_num))
        ...
        interface_prob_pkl_path = os.path.join(interface_dir, 'interface_prob_avg', f"{protein}_{'_'.join(targeted_chains)}-interface-prob-step0.pkl")
        if os.path.exists(interface_prob_pkl_path):
            print(f'[DEBUG-interface]: DatasetTest, loading interface prob.')
            # load from file
            with open(interface_prob_pkl_path, 'rb') as pkl:
                interface_prob = pickle.load(pkl)
        else:
            print(f'[DEBUG-interface]: DatasetTest, Not found interface prob file {interface_prob_pkl_path}.')
            interface_prob = np.zeros((n_token, n_token), dtype=np.float32)
    ...
    def get_sample(self, protein_info, random_seed=None, 
        ...
        # add interface info
        np_example['interface_info_full'] = self.get_full_interface_from_gt_structure(
                                                protein_name, np_example, raw_labels, chain_info_list)
        if self.use_interface_info:
            if self.interface_type == 'dynamic_sampling':
                np_example['interface_prob']  = self.get_interface_prob(protein_name, np_example, chain_info_list)
            else:
                # build interface info
                np_example['interface_info'] = self.get_interface_info(protein_name, np_example, raw_labels, chain_info_list)
...
class AllAtomTestDataset(IterableDataset):
    ...
    def __init__(self,
        ...
        self.interface_config = self.data_config.get("interface_config", 
            {"dist_thres": 5, "top_n": 10, "min_m": 10, "max_m": 10, "seed": 1})
        self.interface_type = self.data_config.get("interface_type", "any_interface")
    ...
    def filter_out_predicted_cases(self, predicted_cases):
        # reload protein infos
        protein_infos, failed_proteins, failed_codes = self._get_protein_infos()
    ...
    def _check_protein(self, protein_info):
        ...
        protein_info: jsonline in {"protein_name": "PROT", "chains": [{"id": "ID"},
         {"id": "ID", "is_ligand": TRUE}, … ]} format
        """
        # print(f'[AllAtomTestDataset]: checking: {protein_info}')
        ...
        if not exists(unit_json_path):
            print(f'[AllAtomTestDataset]: unit_json_path NOT exist: {unit_json_path}')
            return 1 
        if self.use_assembly and not exists(assembly_json_path):
            print(f'[AllAtomTestDataset]: assembly_json_path NOT exist: {assembly_json_path}')
            return 2
        json_dict = json.load(open(assembly_json_path 
                if self.use_assembly else unit_json_path))
        if not 'tokenization' in json_dict: 
            print(f'[AllAtomTestDataset]: tokenization not in json_dict')
            return 3
        if not 'basic' in json_dict: 
            print(f'[AllAtomTestDataset]: basic not in json_dict')
            return 4
        ...
        for chain_id in targeted_chains:
            ...
            if not os.path.exists(features_pkl): 
                print(f'[AllAtomTestDataset] features_pkl {features_pkl} not found')
                return 6

        # Check structure file
        unit_struct_path, assembly_struct_path = self._get_protein_struct_file(protein)
        if not exists(unit_struct_path):
            print(f'[AllAtomTestDataset] unit_struct_path {unit_struct_path} not found')
            return 7
        if self.use_assembly and not exists(assembly_struct_path):
            print(f'[AllAtomTestDataset] assembly_struct_path {assembly_struct_path} not found')
            return 8
        return 0
    ...
    def get_full_interface_from_gt_structure(self, protein, feat, label, chain_info_list):
        ...
        if self.interface_type == "epitope_paratope":
            ...
            if len(antibody_cids) == 0:
                n_token = len(feat['asym_id'])
                none_interface_info = np.zeros([n_token, n_token], dtype=np.float32)
                return none_interface_info
    ...
    def get_interface_info(self, protein, feat, label, chain_info_list):
        ...
        if self.interface_type == "any_interface":
            # random select interface
            # random select m between min_m to max_m
            m = random.randint(self.interface_config['min_m'], self.interface_config['max_m'])
            seed = self.interface_config.get('seed', None)
            sample_times = self.interface_config.get('sample_times', 1)
            interface_info_list = build_interface_info_with_random_residues(
                    ...
                    seed=seed,
                    sample_times=sample_times)
            interface_map_list = [x.interface_map() for x in interface_info_list]
            interface_info = np.stack(interface_map_list, axis=-1).astype("float32")
        elif self.interface_type == "epitope_paratope":
            # select interface between antigen and antibody
            antigen_cids = []
            antibody_cids = []
            for cid, chain_info in enumerate(chain_info_list):
    ...
    def get_sample(self, protein_info):
        ...
        # NOTE: Mainly for analysis, can be turn off for efficiency
        np_example['interface_info_full'] = self.get_full_interface_from_gt_structure(
                                                    protein, np_example, raw_labels, item['chains'])
        if self.use_interface_info:
            if self.interface_type == 'dynamic_sampling':
                np_example['interface_prob']  = self.get_interface_prob(protein, np_example, item['chains'])
            else:
                # build interface info
                np_example['interface_info'] = self.get_interface_info(protein, np_example, raw_labels, item['chains'])
    ...
    def __iter__(self):
        ...
        for x in self.protein_infos:
            try:
                if not self.check_in_blacklist(x):
