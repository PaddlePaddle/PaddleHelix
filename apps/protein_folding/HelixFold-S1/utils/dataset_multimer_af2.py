...
def fasta_to_multimer_af2_feature(data, prot_name, model_config):
...
class MultimerAF2Dataset(paddle.io.Dataset):
    """tbd."""
    def __init__(self,
            model_config,
            data_config,
    ...
    def get_input_feat(self, protein):
        ...
        ### gen_input_feat
        data = {"sequence": raw_features['sequence'][0].decode('utf8')}
        raw_features, seq_infos = fasta_to_multimer_af2_feature(data, protein, self.model_config)
    ...
    def get_sample(self, protein):
        ...
        if chain_num > 10:
            print(f"Skip {protein} due to chain_num > 10")
            return None
    ...
    def __getitem__(self, index):
        ...
        while sample is None:
            ...
            try:
                ...
                print(f'[Multimer AF2 DATA] index {index} prot {protein} failed: {ex}')
                import traceback
...
def demo_multimer_af2():
    import json
    import ml_collections
    from helixfold.model import config
    Model_name = 'multimer_seq512_pair64_l8_vio0'
    model_config = config.model_config(Model_name)
    data_config = ml_collections.ConfigDict(json.load(open('./data_configs/demo_multimer_af2.json', 'r')))
    dataset = MultimerAF2Dataset(
        model_config=model_config,
        data_config=data_config.distill.af2,
        crop_size=100,
        is_pad_if_crop=True,
