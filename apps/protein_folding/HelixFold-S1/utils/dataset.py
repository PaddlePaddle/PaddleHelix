...
from helixfold.data.utils import crop_and_pad
...
class AF2Dataset(paddle.io.Dataset):
    ...
    def _get_protein_struct_file(self, protein):
        ...
        if self.data_config.get('cif_format', '.cif') == ".cif":
            ...
            protein_label_file = os.path.join(
                    self.data_config.structure_dir, protein_name + '.cif.gz')
    ...
    def get_sample(self, protein):
        ...
        if feat['aatype'].shape[1] != label['aatype_index'].shape[0]:
            ...
            return None
        # crop
        is_left_crop, is_right_crop = False, False
        if not self.crop_size is None and self.crop_size > 0:
            num_res = label['aatype_index'].shape[0]
            feat, label, (crop_start, crop_end) = crop_and_pad(
                    feat, label, crop_size=self.crop_size,
                    pad_for_shorter_seq=self.is_pad_if_crop,
                    return_crop_range=True)
            is_left_crop = crop_start > 0
            is_right_crop = crop_end < num_res
        sample = {
            'name': protein,
            'feat': feat,
            'label': label,
            'extra': {
                'is_left_crop': is_left_crop,
                'is_right_crop': is_right_crop,
            }
        }
        return sample
...
class AF2MSATestDataset(IterableDataset):
    """tbd."""
    def __init__(self,
            model_config,
            data_config,
    ...
    def _get_proteins(self):
        """tbd."""
        proteins = []
        with open(self.data_config.protein_clust_file, 'r') as f:
            for line in f:
                name = line.strip()
                if self._check_protein(name):
    ...
    def _get_protein_feature_file(self, protein):
        """tbd."""
        return join(self.data_config.feature_dir, protein + '.a3m')
    ...
    def get_input_feat(self, protein):
        """tbd."""
        protein_feature_file = self._get_protein_feature_file(protein)
        a3m_str = ''.join(open(protein_feature_file, 'r').readlines())
...
class AF2FastaTestDataset(IterableDataset):
    """tbd."""
    def __init__(self,
            model_config,
            data_config,
    ...
    def _get_protein_feature_file(self, protein):
        """tbd."""
        return join(self.data_config.feature_dir, protein, 'target.fasta')

    def _get_protein_struct_file(self, protein):
        """tbd."""
        cif_file = join(self.data_config.structure_dir, protein, 'target.pdb')
        return cif_file
...
class AFDBDataset(paddle.io.Dataset):
    """tbd"""
    def __init__(self,
            model_config,
            data_config,
    ...
    def get_sample(self, protein):
        ...
        processed_feature_dict = self.get_input_feat(data, prot_name)
        protein_label = self.get_label(data)

        ### crop_and_pad
        is_left_crop, is_right_crop = False, False
        num_res = protein_label['aatype_index'].shape[0]
        feat, label, (crop_start, crop_end) = crop_and_pad(
                processed_feature_dict, protein_label, crop_size=self.crop_size, 
                pad_for_shorter_seq=self.is_pad_if_crop,
                return_crop_range=True)
        is_left_crop = crop_start > 0
        is_right_crop = crop_end < num_res
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f'[DATA] index {index} prot {protein} failed: {ex}', flush=True)
