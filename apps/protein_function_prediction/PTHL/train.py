...
def run_batch(model, optimizer, data_loader, epoch_i, desc, loss_fn):
    ...
    for batch in tqdm(data_loader, desc=f"{desc} Epoch {epoch_i}"):
        # print(f'After epoch {epoch_i}')
        logits, labels = do_compute(model, batch)

        loss = loss_fn(logits, labels)
        loss = paddle.mean(paddle.sum(loss, -1))
        # print(f'After loss computtion {epoch_i}, loss = {paddle.sum(loss)}')
        if model.training:
            loss.backward()
            optimizer.step()
        ...
        # print(f'After backpropagation {epoch_i}')
    ...
    # print(f'After computing metrics {epoch_i}')
...
if __name__ == "__main__":
    data_source = "/home/ssd2/v_kazadiarnold/"
    ...
    parser.add_argument(
        "--train_file",
        type=str,
        default=f"{data_source}/data_splits/nrPDB-GO_2019.06.18_train.txt",
        help="File containing training protein chains.",
    )
    parser.add_argument(
        "--valid_file",
        type=str,
        default=f"{data_source}/data_splits/nrPDB-GO_2019.06.18_valid.txt",
        help="File containing validation protein chains.",
    )
    parser.add_argument(
        "--protein_chain_graphs",
        type=str,
        default=f"{data_source}/data/afs/GEOM-2/data/chain_graphs",
        help="Path to graph reprsentations of proteins.",
    )
    parser.add_argument(
        "--label_data_path",
        type=str,
        default=f"{data_source}/afs/PROT/data/labels/molecular_function.npz",
    ...
    parser.add_argument('--n_h_dim', type=int, default=128, help='Dimension of node hidden embeddings.')
    parser.add_argument('--s_h_dim', type=int, default=64, help='Scalar features - hidden dimensions.')
    parser.add_argument('--v_h_dim', type=int, default=32, help='Vector features - hidden dimensions.')
    parser.add_argument('--v_n_feats', type=int, default=2, help='Number of vector-based features')
    parser.add_argument('--t_out_dim', type=int, default=512, help='Feature dimension of Seq encorder')
    parser.add_argument('--t_n_heads', type=int, default=4, help='Number of attention heads')
    parser.add_argument('--t_n_layers', type=int, default=3, help='Number of seq encoder layers')
