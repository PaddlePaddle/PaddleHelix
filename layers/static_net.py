...
class AlphaFold(object):
  ...
  def __init__(self, 
    ...
    self.alphafold_iteration = StaticAlphaFoldIteration(
        self.config, 
        self.global_config,
        self.channel_num, 
        seq_len,
        n_cpus,
        module_prefix='%s.alphafold_iteration' % module_prefix,
...
class StaticAlphaFoldIteration(object):
    ...
    def __init__(self, 
        ...
        seq_len, 
        n_cpus, 
        module_prefix = 'alphafold_iteration', 
        root_weights = 'static_modules',
        is_pdinfer_init=False
    ):
        super(StaticAlphaFoldIteration, self).__init__()
        ...
        self.evoformer = StaticEmbeddingsAndEvoformer(
          config=self.config['embeddings_and_evoformer'],
          global_config=self.global_config,
          seq_len=seq_len,
          channel_num=self.channel_num,
          n_cpus=n_cpus,
          module_prefix='%s.embeddings_and_evoformer' % module_prefix,
          root_weights=root_weights,
          is_pdinfer_init=is_pdinfer_init)
    ...
    def np2pd(self, d:dict) -> dict:
      res = {}
      for k,v in d.items():
        if isinstance(v, np.ndarray):
          if k == 'aatype':
            res[k] = paddle.Tensor(v, dtype='int32')
          else:
            res[k] = paddle.Tensor(v, dtype='float32')
        else:
          res[k] = v
      return res
            ...
            for head_name, head_config in self._get_heads():
                ...
                if head_name in ('predicted_lddt', 'predicted_aligned_error'):
                    ...
                    if head_name == 'structure_module':
                        ret[head_name] = self.heads[head_name](representations, self.np2pd(batch0))
                    else:
